...
// clang-format off
//
// Idea pilfered from XRPL rippled (getAllLedgerFlags XMACRO): one table
// generates the enum and the name→value maps for server_definitions.
// The rows are Xahau's flags (TSH, URIToken, Remit, EmittedDir, …),
// not a copy of rippled's Vault/Loan set.
...
// LEDGER_OBJECT(TypeName, flags…)
// LSF_FLAG     — define the enumerator
// LSF_FLAG2    — reuse an enumerator already defined (same bits, other type)
#define XMACRO(LEDGER_OBJECT, LSF_FLAG, LSF_FLAG2)                             \
    LEDGER_OBJECT(AccountRoot,                                                 \
        ...
        LSF_FLAG(lsfTshCollect, 0x02000000)                                    \
        ...
        LSF_FLAG(lsfURITokenIssuer, 0x40000000)                                \
        LSF_FLAG(lsfDisallowIncomingRemit, 0x80000000)                         \
        LSF_FLAG(lsfAllowTrustLineClawback, 0x00001000))                       \
                                                                               \
    LEDGER_OBJECT(Offer,                                                       \
    ...
    LEDGER_OBJECT(SignerList,                                                  \
        LSF_FLAG(lsfOneOwnerCount, 0x00010000))                                \
                                                                               \
    LEDGER_OBJECT(DirectoryNode,                                               \
        LSF_FLAG(lsfNFTokenBuyOffers, 0x00000001)                              \
        LSF_FLAG(lsfNFTokenSellOffers, 0x00000002)                             \
        LSF_FLAG(lsfEmittedDir, 0x00000004))                                   \
                                                                               \
    LEDGER_OBJECT(NFTokenOffer,                                                \
        LSF_FLAG(lsfSellNFToken, 0x00000001))                                  \
                                                                               \
    LEDGER_OBJECT(URIToken,                                                    \
        LSF_FLAG(lsfBurnable, 0x00000001))                                     \
                                                                               \
    LEDGER_OBJECT(Remark,                                                      \
        LSF_FLAG(lsfImmutable, 1))                                             \
                                                                               \
